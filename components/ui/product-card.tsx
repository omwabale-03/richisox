...
function useSetActiveProduct(initialColor = 0) {
  const [state, setState] = React.useState({ activeColor: initialColor, activeImage: 0 });
  ...
  const handleMouse = React.useCallback((event: "enter" | "leave") => {
    setState((prev) => ({ ...prev, activeImage: event === "enter" ? 1 : 0 }));
  }, []);
      ...
      {productImages.map((productImage, index) => (
        ...
        >
          <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <AnimatePresence>
              <motion.div key={0} variants={variants} className="pointer-events-none" exit="hidden">
                <Image
              ...
              <motion.div
                ...
                animate={
                  activeImage === 1 && productImage.id === productImages[activeColor].id
                    ? "visible"
                    : "hidden"
                }
                exit="hidden"
              >
                <Image
                  ...
                  fill
                  className="object-contain"
                  src={productImage.images[1] || productImage.images[0]}
                  loading="lazy"
                />
              </motion.div>
            </AnimatePresence>
...
export function ProductCard({ id, images, colors, className }: ProductCardProps) {
